...
#Utility functions for using tensorflow model
...
def output_tensor(interpreter, i):
    ...
    out = interpreter.get_output_details()[i]
    out_data = np.squeeze(interpreter.tensor(out['index'])())
    if 'quantization' not in out:
        return out_data
    scale, zero_point = out['quantization']
    if scale == 0:
        return out_data - zero_point
    return scale * (out_data - zero_point)
...
class BBox(collections.namedtuple('BBox', ['xmin', 'ymin', 'xmax', 'ymax'])): #bounding box
